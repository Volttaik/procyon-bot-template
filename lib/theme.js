...
function loadTheme(name) {
    const key  = (name || 'procyon').toLowerCase().trim();
...
function getActiveTheme() {
    ...
    const strings = loadTheme(config.THEME) || loadTheme('procyon');
...
function fmt(body) {
    const name = getStr('botName') || 'Procyon Bot';
    const foot = getStr('footer')  || 'Powered by Procyon';
